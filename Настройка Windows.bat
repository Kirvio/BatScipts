...
REM Настройка 7zip архиватором по умолчанию
...
"%LOCALAPPDATA%\SumatraPDF\SumatraPDF.exe" -register-for-pdf

REM отключаем автозапуск

      reg add "HKLM\Software\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v NoDriveTypeAutoRun /t REG_DWORD /d 0xFF /f

REM включаем автоматическую подкачку

      wmic computersystem set AutomaticManagedPagefile=True

REM Дефрагментируем диски
